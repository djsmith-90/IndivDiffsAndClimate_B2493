### Script for project 'The role of individual differences in pro-environmental behaviour' - ALSPAC B-number B4293
### Script: Generating synthetic datasets
### Created 4/12/2023 by Dan Major-Smith
### R version 4.3.1
...
setwd("X:\\Studies\\RSBB Team\\Dan\\B4293 - MSc Environmental Behaviors")
...
##########################################################################################
#### Read in the processed dataset, reduce down to complete-case observations, and generate synthetic datasets using 'synthpop' (https://www.synthpop.org.uk/get-started.html)

data_raw <- read.dta13("B4293_ReligionAndClimate_forSynth.dta", nonint.factors = TRUE)
head(data_raw)
str(data_raw)


# Take a working copy by dropping some admin vars and keeping just observations in the complete-case analyses
dat <- data_raw %>%
  select(-c(aln, qlet, total_actions)) %>%
  filter((!is.na(sex) & !is.na(ageAtBirth) & !is.na(home) & !is.na(edu) & !is.na(imd) & 
            !is.na(ethnic) & !is.na(offspringAge)) & 
           (!is.na(extra) | !is.na(agree)  | !is.na(consc) | !is.na(emoStab) | !is.na(open) | !is.na(iq)) &
           (!is.na(climateChanging) | !is.na(climateConcern) | !is.na(climateHumans) | !is.na(climateAction) |
              !is.na(travel) | !is.na(waste) | !is.na(energy) | !is.na(buy) | !is.na(airTravel) |
              !is.na(elecCar) | !is.na(localFood) | !is.na(recycle) | !is.na(plastic) | !is.na(sustainable) |
              !is.na(insulation) | !is.na(solar) | !is.na(veg) | !is.na(trees) | !is.na(avoidFossil) | 
              !is.na(children)  | !is.na(otherAction) | !is.na(meatDairy)))
...
# Create a synthetic dataset using default options (which are non-parametric/CART [classification and regression trees]) - Also specify rules
dat_syn <- syn(dat, seed = 54345,
...
pdf("./Results_SynthPop/ComparingDescStats.pdf", height = 18, width = 20)
compare(dat_syn, dat, stat = "counts", nrow = 7, ncol = 8)
dev.off()


## Univariable analysis with reducing meat/dairy consumption as outcome and IQ/cognitive ability as exposure to show that get similar results in both datasets (i.e., that the structures of the dataset are preserved)
model.syn <- multinom.synds(meatDairy ~ iq, data = dat_syn)
summary(model.syn)
...
model.syn2 <- multinom.synds(meatDairy ~ iq + sex + ageAtBirth + home + edu + imd + ethnic, data = dat_syn)
summary(model.syn2)
...
## Now re-create the total number of climate actions variable
dat_syn$syn <- dat_syn$syn %>%
  rowwise() %>%
...
write_dta(dat_syn_df, ".\\AnalysisCode_B4293\\SyntheticData\\syntheticData_B4293.dta")
